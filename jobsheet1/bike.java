package jobsheet1;

public class bike {
    private String brand;
    private int speed;
    private int gear=1;

    // Gear 1: max 5 km/h, Gear 2: max 10 km/h, .. Gear 6: max 60 km/h
    private final int[] GEAR_SPEED_LIMITS = {5, 10, 25, 30, 40, 60};

    //modif
    public static int totalSepeda = 0;
    public bike() {
        totalSepeda++;
    }

    public void setBrand(String brandName) {
        brand = brandName;
    }

    public void gearChanges(int gearValue) {
        if (gearValue<1 || gearValue>6) {
            System.out.println("Invalid gear value. Gear must be between 1 and 6.");
        } else {
            gear = gearValue;
        }
    }   

    public int speedAcceleration(int increment) {
        speed += increment;
        //ngecdek limit jika gear sudah diset (> 0)
        if (gear > 0 && speed > GEAR_SPEED_LIMITS[gear-1]) {
            speed = GEAR_SPEED_LIMITS[gear-1];
        }
        // if (speed > GEAR_SPEED_LIMITS[gear-1]) {
        //     speed = GEAR_SPEED_LIMITS[gear-1];
        // }
        return speed;
    }

    public int speedDeceleration(int decrement) {
        speed -= decrement;
        if (speed<0) {
            speed=0;
        }
        return speed;
    }

    //modif
    public static void tampilkanTotalSepeda() {
        System.out.println("Total Sepeda Aktif: " +totalSepeda +"unit");
    }

    public void printInfo() {
        System.out.println("Brand : " +brand);
        System.out.println("Speed : " +speed);
        System.out.println("Gear  : " +gear);
    }
}
